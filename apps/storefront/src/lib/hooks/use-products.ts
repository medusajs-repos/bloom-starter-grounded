  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { products } = await sdk.store.product.list({
        ...
        region_id,
        fields: fields ||
          "*variants, +variants.inventory_quantity, +variants.manage_inventory, +variants.allow_backorder, +variants.images, *images, *options, *options.values, *collection, *tags",
      })
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await sdk.store.product.list({
        ...
        order: "-created_at",
        region_id,
        fields: "*variants, *variants.images",
      })
...
export const useProductCount = ({
  ...
  return useQuery({
    queryKey: queryKeys.products.count(region_id),
    queryFn: async () => {
      const response = await sdk.store.product.list({
        limit: 1,
        offset: 0,
